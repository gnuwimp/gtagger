...
import gnuwimp.core.extension.isAudio
import gnuwimp.core.extension.isImage
import gnuwimp.core.extension.swap
...
import gnuwimp.core.swing.extension.toImageIcon
import gnuwimp.core.util.*
...
object Data {
    ...
    var messageFunc: (String) -> Unit = { _: String -> Unit }
    ...
    fun filterOnAlbums(key: String) {
        tracks.forEach { track ->
            track.isSelected = track.artist.indexOf(string = key, ignoreCase = true) >= 0 ||
    ...
    fun loadIconFromFile(file: File?): ImageIcon {
        return try {
            if (file == null || !file.isFile)
                ...
                ImageIO.read(file).toImageIcon(Text.ICON_SIZE.toDouble())
        }
        catch (e: Exception) {
    ...
    fun loadImageForSelectedTrack(pref: Preferences) {
        ...
        if (track != null) {
            try {
                ...
                if (file != null && file.isFile && file.isImage) {
    ...
    fun loadTracks(newPath: String) {
        ...
        val tasks = mutableListOf<AudioReadTask>()

        path = newPath
        message = Text.MESSAGE_LOADING_TRACKS.format(path)
        tracks = listOf()
        selectedRow = -1

        File(path).listFiles()?.filter(File::isAudio)?.forEach { file ->
            tasks.add(AudioReadTask(file = file, fileName = file.name))
        }

        LogManager.getLogManager().reset()

        if (tasks.size > 0) {
            val progress = Progress(tasks = tasks, threadCount = 3)
            val dialog = ProgressDialog(progress = progress, title = Text.DIALOG_TITLE_LOAD, parent = Main.window)

            dialog.enableCancel = true
            dialog.enablePause = true
            dialog.start(actionOnError = ProgressAction.CONTINUE, actionOnCancel = ProgressAction.JOIN)

            val stat = Text.MESSAGE_TIME.format("${ System.currentTimeMillis() - start}")

            message = if (tasks.countError == 0)
                Text.MESSAGE_LOADING.format(tasks.tracks.size, path, stat)
            else
                Text.ERROR_LOADING.format(tasks.tracks.size, tasks.countError, path, stat)

            tracks = tasks.tracks

            tasks.allErrors().forEach { task ->
                Platform.logMessage = task.error
            }
    ...
    fun removeTags() {
        try {
            ...
            Platform.logMessage = Text.ERROR_DELETE_TAGS.format(selectedTrack?.fileNameWithExtension)
            JOptionPane.showMessageDialog(Main.window, Text.ERROR_DELETE_TAGS1_HTML.format(selectedTrack?.fileNameWithExtension), Text.DIALOG_CLEAR_FAILED, JOptionPane.ERROR_MESSAGE)
        }
        finally {
            sendUpdate(TrackEvent.ITEM_SELECTED)
        }
    ...
    fun removeTagsForAll() {
        ...
        tracks.filter { it.isSelected }.forEach { track ->
            try {
                track.clear()
            }
            catch (e: Exception) {
                Platform.logMessage = Text.ERROR_DELETE_TAGS.format(track.fileNameWithExtension)
                failed = true
            }
        }
        ...
        if (failed)
            JOptionPane.showMessageDialog(Main.window, Text.ERROR_DELETE_TAGS2_HTML.format(selectedTrack?.fileNameWithExtension), Text.DIALOG_CLEAR_FAILED, JOptionPane.ERROR_MESSAGE)
    }
    ...
    fun saveChangedAskFirst(displayErrorDialogOnSave: Boolean, abortOnFailedSave: Boolean): Boolean {
        if (isAnyChangedAndSelected &&
            ...
            if (displayErrorDialogOnSave)
                JOptionPane.showMessageDialog(Main.window, Text.ERROR_SAVE1_HTML, Text.DIALOG_SAVE_FAILED, JOptionPane.ERROR_MESSAGE)

            if (abortOnFailedSave)
                return false
        }
    ...
    fun saveTracks(): Boolean {
        val start = System.currentTimeMillis()
        val tasks = mutableListOf<AudioSaveTask>()

        tracks.filter {
            it.isChanged && it.isSelected
        }.forEach {
            tasks.add(AudioSaveTask(track = it, fileName = it.fileName))
        }

        if (tasks.size > 0) {
            val progress = Progress(tasks = tasks, threadCount = 3)
            val dialog = ProgressDialog(progress = progress, title = Text.DIALOG_TITLE_SAVE, parent = Main.window)
            ...
            message = if (tasks.countError == 0)
                Text.MESSAGE_SAVING.format(tasks.size, stat)
            else
                Text.ERROR_SAVING.format(tasks.countOk, tasks.countError, stat)
    ...
    fun sortTracks(sortTracksOn: Sort, descending: Boolean = false) {
        val list = tracks.sortedWith (Comparator { track1, track2 ->
            when (sortTracksOn) {
